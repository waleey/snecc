...
def load_events(bulkice_outfile_path):
    """
    Load bulkice CSV output into Event objects.

    Memory/speed improvement over the old version:
      - read only columns actually used by this function;
      - avoid DataFrame.iterrows(), which creates a Series object per photon row;
      - keep the same Event/Photon construction and multiplicity calculation.
    """
    usecols = [
        "run_id", "enu", "ee",
        "vertex_x", "vertex_y", "vertex_z",
        "photon_energy", "pmt_hit", "hit_time"
    ]
    master_df = pd.read_csv(bulkice_outfile_path, usecols=usecols)
    HC = 1241  # eV·nm

    events = []

    # Keep pandas' default sorted group order to match the previous groupby behavior.
    for run_id, group in master_df.groupby('run_id', sort=True):
        first = group.iloc[0]
        ...
        photon_energy = group['photon_energy'].to_numpy(copy=False)
        pmt_hits = group['pmt_hit'].to_numpy(copy=False)
        hit_times = group['hit_time'].to_numpy(copy=False)
        ...
        photons = [
            Photon(
                ...
                pmt=pmt,
                runID=runID,
                time=(hit_time * 1e-9 - 1e-3)
            )
            for energy_eV, pmt, hit_time in zip(photon_energy, pmt_hits, hit_times)
        ]
...
def flux_weight_scalar(snewpy_distribution, Ngen=600549, distance=5 * u.kpc, radius=20 * u.m):
    """
    Same scalar calculation as flux_weight(), but returns the multiplicative
    flux factor instead of looping over Event objects and mutating weights.
    """
    E_grid = (snewpy_distribution['energy_grid'] * u.MeV).to(u.J).value
    spectra = (snewpy_distribution['spectra_rebinned'] * u.MeV).to(u.J).value
    time_grid = (snewpy_distribution['time_grid'] * u.s).value
    ...
    for j in range(spectra.shape[1]):
        ...
        if np.isnan(num) or np.isnan(denom) or denom == 0:
            ...
            avg_energy.append(num / denom)
        avg_energy_square.append(
            np.trapezoid((E_grid**2) * spectra[:, j], E_grid) / denom
            if denom != 0 else 0.0
        )
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        energy_averaged_flux = np.divide(flux_vals, avg_energy)
        energy_averaged_flux[~np.isfinite(energy_averaged_flux)] = 0.0

    flux_factor = (1 / Ngen) * (radius**2 / (distance**2)) * np.trapezoid(energy_averaged_flux, time_grid)
    return flux_factor, model_avg_energy * 6.242e12, model_alpha


def cross_section_weight_array(base_enu):
    """
    Same calculation as cross_section_weight(), but returns an array instead
    of mutating Event objects.
    """
    ibd = InvBetaPar()
    l = (40 * u.m).to(u.cm).value
    ice_density = 0.92
    ...
    weights = np.empty(len(base_enu), dtype=np.float64)
    for i, enu in enumerate(base_enu):
        xs = ibd.cross_section(Flavor.NU_E_BAR, enu * u.MeV)
        weights[i] = xs.value * n_target * l
    return weights


def effective_volume_weight_array(base_mult, abs_file_path, absorption_sim, N_modules, max_multiplicity, multiplicity_params):
    """
    Same W_eff(m) calculation as eff_vol_weight(), but returns an array
    aligned with base_mult instead of mutating Event objects.
    """
    with open(abs_file_path, 'r') as f:
        absorption_lengths = [float(line.strip()) for line in f if line.strip()]
    ...
    for m in range(1, max_multiplicity + 1):
        ...
        V_eff_mean = np.mean(V_eff_all)
        V_eff_sim = b * (1.0 / absorption_sim) + c
        weights_by_m[m] = N_modules * (V_eff_mean / V_eff_sim)

    fallback = weights_by_m[max(weights_by_m)]
    return np.array([weights_by_m.get(int(m), fallback) for m in base_mult], dtype=np.float64)
...
def calculate_coincidence_ratio(
    ...
    This version is memory-optimized but keeps the same calculation:
      old path: Event objects -> deep copy -> mutate .weight repeatedly
      new path: Event objects -> compact numpy arrays -> same weights

    The physics/analysis steps are unchanged:
      cross-section weight × flux weight × effective-volume weight × spectrum weight.
    """
    ...
    # Build compact base arrays instead of deep-copying every Event and Photon.
    base_enu = np.array([e.enu for e in events], dtype=np.float64)
    base_mult = np.array([e.multiplicity for e in events], dtype=np.int64)

    base_weight0 = cross_section_weight_array(base_enu)

    flux_factor, model_avg_energy, model_alpha = flux_weight_scalar(
        snewpy_distribution=snewpy_distribution,
        Ngen=Ngen,
        distance=distance,
        radius=radius
    )
    base_weight0 *= flux_factor
    ...
    base_weight0 *= effective_volume_weight_array(
        base_mult=base_mult,
    ...
    base_weight0 = np.nan_to_num(base_weight0, nan=0.0, posinf=0.0, neginf=0.0)

    multiplicities = range(multiplicity_min, multiplicity_max + 1)

    ratio_grids = {m: np.zeros((len(mean_energies), len(alphas))) for m in multiplicities}
    m_weights = {m: np.zeros((len(mean_energies), len(alphas))) for m in multiplicities}
    gt_weights = {m: np.zeros((len(mean_energies), len(alphas))) for m in multiplicities}
    ...
    for i, meanE in enumerate(mean_energies):
        for j, alpha in enumerate(alphas):
            ...
            weights = spectrum_weight_array(base_enu, base_weight0, energy_grid, normalized_spectrum)
...
"""
def _percentile_from_partitioned_1d(arr, q_values):
    """
    Compute NumPy-style linear percentiles from a 1D ndarray/memmap using
    in-place partitioning. This avoids a second full-size sorted copy.
    """
    q_values = list(q_values)
    n = arr.shape[0]
    if n == 0:
        return [np.nan for _ in q_values]

    positions = [(q / 100.0) * (n - 1) for q in q_values]
    kth = sorted(set(int(np.floor(p)) for p in positions) | set(int(np.ceil(p)) for p in positions))
    arr.partition(kth)
    ...
    for p in positions:
        ...
        if lo == hi:
            ...
            frac = p - lo
            out.append(float(arr[lo] * (1.0 - frac) + arr[hi] * frac))
    return out
...
def calculate_combined_confidence_intervals(
    ...
    """
    Calculate combined chi-square confidence intervals.

    store_samples=True reproduces the old behavior and returns the full flattened
    chi-square array. store_samples=False writes that same flattened sequence to
    a temporary disk-backed memmap, computes the percentiles, deletes the memmap,
    and returns None for the samples. This is the memory-safe mode for production
    runs where only ci68/ci99 are saved.
    """
    import tempfile
    ...
    n_A = len(alphas)
    total_size = n_E * n_A * num_simulations

    tmp_path = None
    if store_samples:
        chi_squares_flat = np.empty(total_size, dtype=np.float64)
    else:
        fd, tmp_path = tempfile.mkstemp(prefix="snecc_chisq_", suffix=".dat", dir=tmp_dir)
        os.close(fd)
        chi_squares_flat = np.memmap(tmp_path, dtype=np.float64, mode="w+", shape=(total_size,))
    ...
    if not store_samples:
        chi_squares_flat.flush()
        ci68, ci99 = _percentile_from_partitioned_1d(chi_squares_flat, [68, 99])
        del chi_squares_flat
        if tmp_path is not None:
            try:
                os.remove(tmp_path)
            except OSError:
                pass
        return None, ci68, ci99

    ci68 = np.percentile(chi_squares_flat, 68)
    ci99 = np.percentile(chi_squares_flat, 99)

    return chi_squares_flat, ci68, ci99
